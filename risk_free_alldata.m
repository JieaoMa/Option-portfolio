...
for year = 1:7
    ...
    for i = 1:mon(year,1)
        ...
        S0_50 = CP(i,1);
        Lower = S0_50*(1-varphi*sigma);   
        Upper = S0_50*(1+varphi*sigma);
    ...
    %---------Solve-----------
        ...
        cvx_begin
        variables xf(1) x_c(n_50) x_p(n_50)
        maximize ((1+rf)*xf+(C1)'*x_c+(P1)'*x_p)
        xf >= 0;
        xf + OF_c'*x_c + OF_p'*x_p == 1;
        norm(x_c,1)+norm(x_p,1) <= 5;
        for S = linspace(Lower,Upper,100)
            for i_3 = 1:n_50
                if S > K50(i_3)
                    rc50(i_3,1) = S-K50(i_3,1); 
                    rp50(i_3,1) = 0; 
                else
                    rc50(i_3,1) = 0; 
                    rp50(i_3,1) = K50(i_3,1)-S; 
                end
            end 
            (rc50)'*x_c+(rp50)'*x_p+(1+rf)*xf >= 1; 
        end
    cvx_end
